...
mov=VideoReader('/home/wzg/Matlab_Code/FirstWork/data/v2_down1.avi');
...
for i=1:num
    ...
    member_num=obj_struct(i).memberNum;
    start=start+125;
    time(member(1))=start;
...
length=10200;
wobj=VideoWriter('data/processed1ID.avi');
wobj.FrameRate=12;
open(wobj);
for z = 1:length
    ...
    for k=1:numel(id)
        ...
        target_id=id(k);
        target_gid=gid(k);
        tmp_image=read(original_mov,2*n);
        id_tube=TTube(id(k));
        ...
        bg = insertObjectAnnotation(bg,'rectangle',[objects(v,3:4) 0 0],['Id:',num2str(target_id),' Gid:',num2str(target_gid), ' N:',num2str(n),' ', num2str(an1),':',num2str(an2),':',num2str(an3)],'Color','red','TextBoxOpacity',0,'Fontsize',12);
